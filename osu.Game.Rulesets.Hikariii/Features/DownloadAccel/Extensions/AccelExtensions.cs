...
namespace osu.Game.Rulesets.Hikariii.Features.DownloadAccel.Extensions
{
            ...
            foreach (char c in url)
            {
                ...
                if (c == ']' && propertyDetectState >= 1)
                {
                    ...
                    if (!vaild_properties.Contains(propertyInfo.name) && !noSuggestion)
                    {
                        ...
                        {
                            if (!propertyName.StartsWith(propertyInfo.name, StringComparison.Ordinal)) continue;
                            haveSuggestion = true;
                    ...
                    if (!string.IsNullOrEmpty(propertyInfo.extensionName))
                    {
                        ...
                        if (!noSuggestion)
                        {
                            ...
                            {
                                if (!hdlr.ExtensionName.StartsWith(propertyInfo.extensionName, StringComparison.Ordinal)) continue;
                                haveSuggestion = true;
                ...
                switch (propertyDetectState)
                {
                    //添加字符到名称
                    case 1:
                        propertyInfo.name = propertyInfo.name + c;
                        break;

                    case 2:
                        propertyInfo.extensionName = propertyInfo.extensionName + c;
                        break;
                }
            }

...
namespace osu.Game.Rulesets.Hikariii.Graphics.Settings
{
    public sealed partial class HikariiiMainSection : RulesetSettingsSubsection
    {
        ...
        private void load(SettingsOverlay settingsOverlay, LLinGlobalConfigManager config)
        {
            AddRange(
            [
                new SettingsItemV2(new FormCheckBox
                {
                    ...
                    Current = config.GetBindable<bool>(LLinGlobal.InjectButtonToNewSongSelect)
                }),
                new SettingsItemV2(new FormCheckBox
                {
                    Caption = "启用系统主题色支持",
                    HintText = "如果当前系统受支持，则会基于系统主题色设定播放器的界面主题色",
                    Current = config.GetBindable<bool>(LLinGlobal.UsePlatformAccentColor)
                }),
                new SettingsItemV2(new FormCheckBox
                {
                    Caption = "启用勿扰模式支持",
                    HintText = "如果当前系统受支持，则会在游玩时启用系统的勿扰模式",
                    Current = config.GetBindable<bool>(LLinGlobal.EnableOSDoNotDisturbWhenPlaying)
                }),
                new SettingsItemV2(new FormCheckBox
                {
                    Caption = "取得焦点时不要响应系统媒体按键",
                    HintText = "当窗口取得焦点时，Hikariii 将不会对系统媒体按键做出响应\n修复一些平台上按下媒体按键会响应两次的问题",
                    Current = config.GetBindable<bool>(LLinGlobal.IgnoreMediaControlWhenFocused)
                }),
                new SettingsItemV2(new FormSliderBar<float>()
                {
                    Caption = "界面主题色（红）",
                    HintText = "若没有平台主题色可用，将根据这些设置设定界面主体色",
                    Current = config.GetBindable<float>(LLinGlobal.AccentRed),
                    KeyboardStep = 1f
                }),
                new SettingsItemV2(new FormSliderBar<float>
                {
                    Caption = "界面主题色（绿）",
                    HintText = "若没有平台主题色可用，将根据这些设置设定界面主体色",
                    Current = config.GetBindable<float>(LLinGlobal.AccentGreen),
                    KeyboardStep = 1f
                }),
                new SettingsItemV2(new FormSliderBar<float>
                {
                    Caption = "界面主题色（蓝）",
                    HintText = "若没有平台主题色可用，将根据这些设置设定界面主体色",
                    Current = config.GetBindable<float>(LLinGlobal.AccentBlue),
                    KeyboardStep = 1f
                }),
                new ColourPreviewer()
            ]);
